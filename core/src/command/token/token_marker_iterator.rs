...
impl<'a> TryFrom<&'a MatchMeta<'a>> for TokenMatchSet<'a> {
    type Error = ();

    fn try_from(input: &'a MatchMeta<'a>) -> Result<Self, Self::Error> {
        match input {
            MatchMeta::Sequence(vec) => Ok(TokenMatchSet::Slice(vec.as_slice())),
            MatchMeta::Single(single) => Ok(TokenMatchSet::Single(single.as_ref())),
            _ => Err(()),
        }
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(token_match) = self.token_match_set.get(self.cursor) {
                if let Ok(token_match_set) = (&token_match.match_meta).try_into() {
