            ...
            for await fuzzy_match in test_token.match_input(input, app_meta) {
                match fuzzy_match {
                    ...
                    FuzzyMatch::Overflow(token_match, remainder) => {
                        ...
                        for await next_fuzzy_match in match_input_with_tokens(token, remainder.as_str(), app_meta, next_tokens) {
...
mod test {
    ...
    async fn match_input_test() {
        ...
        let phrase = [
            Token::keyword_marked("badger", Marker::Keyword),
            Token::any_word_marked(Marker::AnyWord),
        ];
        let [keyword_token, any_word_token] = &phrase;

        let phrase_token = Token::any_of_marked(&phrase[..], Marker::Phrase);
