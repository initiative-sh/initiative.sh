...
impl FromStr for ParsedThing<Place> {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        let description = if let Some((name, description)) = split_name(input) {
            place.data.name = Field::new(capitalize(name));
            description
        } else {
            input
        };
        ...
        for word in quoted_words(description) {
            ...
            if word_str.in_ci(&["a", "an"]) {
                ...
                place.data.subtype = Field::new(place_type);
            } else {
...
impl FromStr for ParsedThing<Npc> {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        let description = if let Some((name, description)) = split_name(input) {
            npc.data.name = Field::new(capitalize(name));
            description
        } else {
            input
        };
        ...
        for word in quoted_words(description) {
            ...
            if word_str.in_ci(&["a", "an"]) {
                ...
                npc.data.gender = Field::new(gender);
                ...
                if let Ok(age) = word_str.parse() {
                    ...
                    npc.data.age.replace(age);
                    npc.data.age.lock();
                }
            } else if let Ok(age) = word_str.parse() {
                npc.data.age = Field::new(age);
            } else if let Ok(species) = word_str.parse() {
                npc.data.species = Field::new(species);

                if let Ok(ethnicity) = word_str.parse() {
                    npc.data.ethnicity.replace(ethnicity);
                    npc.data.ethnicity.lock();
                }
            } else if let Ok(ethnicity) = word_str.parse() {
                npc.data.ethnicity = Field::new(ethnicity);
            } else if let Some(Ok(age_years)) =
                word_str.strip_suffix_ci("-year-old").map(|s| s.parse())
            {
                npc.data.age_years = Field::new(age_years);
            } else {
                unknown_words.push(word.range().to_owned());
            }
        }
...
mod test {
    ...
    fn place_from_str_test() {
...

        {
            ...
            assert_eq!(
                Some("Foo bar"),
                place.thing.data.name.value().map(|s| s.as_str()),
            );
            assert_eq!(0, place.unknown_words.len());
            assert_eq!(1, place.word_count);
        }
...

        {
            ...
            assert_eq!(Some(&PlaceType::Any), place.thing.data.subtype.value());
            assert_eq!(0, place.unknown_words.len());
            assert_eq!(1, place.word_count);
        }
    }
    ...
    fn npc_from_str_test() {
...

        {
            ...
            assert_eq!(Field::Locked(Some(Species::Elf)), npc.thing.data.species);
            assert_eq!(0, npc.unknown_words.len());
            assert_eq!(1, npc.word_count);
        }
        assert_eq!(
...

        {
            ...
            assert_eq!(Field::Locked(Some("Sue".to_string())), npc.thing.data.name);
            assert_eq!(
                Field::Locked(Some(Gender::Masculine)),
                npc.thing.data.gender
            );
            assert_eq!(Field::Locked(Some(Age::Child)), npc.thing.data.age);
            assert_eq!(Field::Locked(Some(37)), npc.thing.data.age_years);
            assert_eq!(0, npc.unknown_words.len());
            assert_eq!(2, npc.word_count);
        }
        assert_eq!(
